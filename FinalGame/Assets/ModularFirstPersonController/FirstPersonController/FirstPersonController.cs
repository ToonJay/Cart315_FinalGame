...
public class FirstPersonController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if(isGrounded) { 
                targetVelocity.x = Input.GetAxis("Horizontal") * walkSpeed;
                targetVelocity.z = slideSpeed;
            } else
            {
    ...
    private void Jump()
    {
        ...
        if (isGrounded)
        {
            rb.AddForce(0f, jumpPower, 0f, ForceMode.VelocityChange);
            isGrounded = false;
        }
...
#region Movement Setup
        ...
        fpc.slideSpeed = EditorGUILayout.Slider(new GUIContent("Slide speed", "Determines how fast the player slides down the slope."), fpc.slideSpeed, .1f, 100);
        GUI.enabled = true;
...
#region Jump
        ...
        fpc.jumpPower = EditorGUILayout.Slider(new GUIContent("Jump Power", "Determines how high the player will jump."), fpc.jumpPower, .1f, 100f);
        GUI.enabled = true;
